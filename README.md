...
Expert proficiency with 0 months used — "expert" skill but duration_months == 0
Duration > total experience — a single job's duration exceeds the candidate's stated total YOE by > 1 year
Duration ≠ date span — duration_months exceeds the start_date → end_date span by > 12 months
...
🧾 Assumptions
No hackathon-provided output spec beyond the validator script was found — defaulted to candidate_id,rank,score,reasoning CSV per submission_spec.docx.
Projects are distinguished from skills by source field (career_history[].description vs skills[].name) — no heuristics needed since the dataset is structured.
Honeypot detection uses deterministic profile inconsistency checks (not ML-based) to ensure reproducibility and auditability.
The reference date for "days since last active" calculations is 2026-06-01 based on the observed max last_active_date in the dataset.
